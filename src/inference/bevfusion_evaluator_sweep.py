...
class BEVFusionEvaluator:
    ...
    def load_points_with_sweeps(self, lidar_data, sweeps_num=9,
        ...
        """
        对齐 mmdet3d LoadPointsFromFile + LoadPointsFromMultiSweeps 的实现。
        加载当前关键帧点云，并将历史 sweeps_num 帧 sweep 点云变换到当前 LiDAR
        坐标系后拼接，完全复现训练时的输入。

        参数:
            lidar_data        : NuScenes sample_data 记录（LIDAR_TOP 关键帧）
            sweeps_num        : 历史 sweep 帧数，默认 9（共 10 帧）
            remove_close_radius: 移除近处点半径阈值（米），对应 remove_close=True

        返回:
            points: shape (N, 5)  [x, y, z, intensity, time_lag]
                    当前帧 time_lag=0，历史帧 time_lag 为与当前帧的秒级时间差

        实现细节（对齐 mmdet3d）：
          1. 当前帧坐标系作为参考坐标系，time_lag=0。
          2. 对每帧 sweep：
               T_rel = inv(T_ref_lidar2global) @ T_sweep_lidar2global
             将 sweep 点云变换到当前 LiDAR 坐标系。
          3. 时间差：(ref_timestamp - sweep_timestamp) * 1e-6  （微秒 → 秒）
          4. remove_close：去掉 sqrt(x²+y²) < remove_close_radius 的点，
             与训练时 LoadPointsFromMultiSweeps(remove_close=True) 一致。
        """
        nusc = self.nusc

        # ----------------------------------------------------------------
        # 1. 当前关键帧 lidar2global 变换矩阵
        # ----------------------------------------------------------------
        ...
        pts_current[:, 4] = 0.0  # 当前帧 time_lag = 0，与训练一致

        # remove_close：去掉雷达正下方附近的点（训练时同步执行）
        ...
        while cur_sd_token != '' and loaded_sweeps < sweeps_num:
            ...
            # T_rel: sweep LiDAR → current LiDAR
            T_rel = T_ref_inv @ T_sweep   # (4, 4)

            # 时间差（秒），对齐训练时的 time_lag 通道
            time_lag = (ref_timestamp - sweep_sd['timestamp']) * 1e-6  # 微秒 → 秒
            ...
            # 坐标变换：sweep LiDAR → current LiDAR
            pts_xyz1 = np.hstack([pts_sweep[:, :3],
                                   np.ones((pts_sweep.shape[0], 1), dtype=np.float32)])
            pts_xyz_ref = (T_rel @ pts_xyz1.T).T[:, :3].astype(np.float32)

            # 组装：[x_ref, y_ref, z_ref, intensity, time_lag]
            pts_sweep_ref = np.hstack([
                pts_xyz_ref,
                pts_sweep[:, 3:4],                                          # intensity
                np.full((pts_sweep.shape[0], 1), time_lag, dtype=np.float32)  # time_lag
            ])

            all_points.append(pts_sweep_ref)
            loaded_sweeps += 1
            print(f"  [Sweep] sweep -{loaded_sweeps}: {pts_sweep_ref.shape[0]} pts, "
                  f"time_lag={time_lag:.4f}s")

            cur_sd_token = sweep_sd['prev']  # 继续向前遍历

        if loaded_sweeps < sweeps_num:
            print(f"  [Sweep] 警告：只找到 {loaded_sweeps} 帧历史 sweep（期望 {sweeps_num}），"
                  f"可能是序列起始帧。")
        ...
        print(f"  [Sweep] 聚合完成：共 {len(all_points)} 帧，总点数 {points.shape[0]}")
        return points
    ...
    def print_timing_summary(self):
        ...
        task_order = ['sweep_loading', 'voxelization', 'inference', 'decode', 'nms', 'transform', 'total_per_sample']
        task_names_cn = {
            'sweep_loading':    'Sweep聚合',
            'voxelization':     '体素化',
            'inference':        '模型推理',
            'decode':           '结果解码',
            'nms':              'NMS处理',
    ...
    def run_evaluation(self, net):
        ...
        for sample in self.nusc.sample:
            ...
            # ------------------------------------------------------------------
            # Sweep 聚合：加载当前帧 + 9 帧历史 sweep，对齐训练时的多帧输入
            # 对应配置：LoadPointsFromMultiSweeps(sweeps_num=9, remove_close=True)
            # ------------------------------------------------------------------
            sweep_start = time.time()
            points = self.load_points_with_sweeps(
                lidar_data,
                sweeps_num=9,          # 与训练配置 sweeps_num=9 保持一致
                remove_close_radius=1.0  # 与训练配置 remove_close=True 保持一致
            )
            self.timing_stats['sweep_loading'].append(time.time() - sweep_start)
            ...
            # Decode outputs
            raw_boxes = self.decode_outputs(outputs, score_threshold=0.01)
            # 打印解码后的框数量以验证
