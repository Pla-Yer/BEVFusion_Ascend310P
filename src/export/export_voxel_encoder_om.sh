...
# BEVFusion 体素编码器 OM 模型导出脚本
# 使用方法: bash export_voxel_encoder_om.sh
...
echo "开始导出 OM 模型（支持动态shape）..."
echo "输入模型: $ONNX_MODEL"
echo "输出目录: $OUTPUT_DIR"
echo "输出名称: $OUTPUT_NAME"
echo "动态档位: 6000, 8000, 10000"

# 使用atc工具导出OM模型，支持动态shape
# input_shape: 定义每个输入的形状，使用-1表示动态维度
# dynamic_dims: 指定动态维度的具体档位
# 格式: 每个档位用分号分隔，每个档位中每个动态维度的值用逗号分隔
# "dim0_val1,dim1_val1,dim2_val1;dim0_val2,dim1_val2,dim2_val2;..."
