...
# BEVFusion TransFusionHead 检测头 OM 模型导出脚本
# 使用方法: bash export_transfusion_head_om.sh
...
# 设置输入输出路径
ONNX_MODEL="$WORK_DIR/models/onnx/transfusion_head.onnx"
OUTPUT_DIR="$WORK_DIR/models/om"
OUTPUT_NAME="transfusion_head"
...
if [ ! -f "$ONNX_MODEL" ]; then
    ...
    echo "请先运行 export_head_onnx.py 导出ONNX模型"
    exit 1
fi
...
atc \
  ...
  --input_format=NCHW \
  --input_shape="bev_feat:1,512,180,180" \
