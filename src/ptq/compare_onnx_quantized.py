...
def run_onnx_model(model_path, inputs):
    ...
    Returns:
        模型输出列表
    """
    # 尝试使用CPU执行提供程序，如果失败则尝试其他提供程序
    try:
        session = ort.InferenceSession(model_path, providers=['CPUExecutionProvider'])
    except Exception as e:
        print(f"CPU执行提供程序失败: {e}")
        try:
            session = ort.InferenceSession(model_path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        except Exception as e2:
            print(f"CUDA执行提供程序也失败: {e2}")
            # 如果还是失败，我们仍然使用CPU执行提供程序，但在推理时捕获异常
            session = ort.InferenceSession(model_path, providers=['CPUExecutionProvider'])
    ...
    try:
        ...
        print(f"模型推理失败: {e}")
        print("量化模型可能包含自定义算子，无法在当前环境下运行")
        raise
...
def compare_models(original_model_path, quantized_model_path, inputs, output_dir):
    ...
    """
    print("运行原始模型...")
    original_outputs = run_onnx_model(original_model_path, inputs)
    
    print("运行量化模型...")
    quantized_outputs = run_onnx_model(quantized_model_path, inputs)
    ...
    for key in original_keys:
        ...
        detailed_results[key] = {
            ...
            'mean_relative_error': float(mean_abs_error),
...
def build_lidar_dummy_inputs(max_voxels=8000, max_points=32, point_dim=5):
    ...
    num_points = np.random.randint(1, max_points + 1, size=(V,)).astype(np.float32)
    coords_zy = np.random.randint(0, 360, size=(V, 2)).astype(np.float32)
    batch_col = np.zeros((V, 1), dtype=np.float32)
    z_col = np.zeros((V, 1), dtype=np.float32)
...
def main():
    parser = argparse.ArgumentParser(description='ONNX模型与量化后模型精度对比')
