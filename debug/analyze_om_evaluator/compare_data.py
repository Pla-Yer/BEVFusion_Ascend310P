...
class DataComparator:
    ...
    def __init__(self, om_data_dir, pytorch_data_dir, output_dir='./analysis_results'):
        """
        初始化对比器
    ...
    def compare_heatmap(self, pt_hm, om_hm, atol=1e-2):
        """
        ...
        ok = np.allclose(pt, om, atol=atol, rtol=atol)
    ...
    def compare_high_conf(self, pt_outputs, om_outputs, score_thresh=0.1, center_tol=2.0):
        """
        对高置信目标做匹配对比：
        - 对每个PT高置信预测，在OM输出中找最近邻（按center距离）
        - 报告命中率和bbox误差
        """
        ...
        # 距离矩阵 [N_pt, N_om]
        diff_mat = pt_centers[:, None, :] - om_centers[None, :, :]  # [N_pt, N_om, 2]
        dist_mat = np.linalg.norm(diff_mat, axis=-1)                 # [N_pt, N_om]
        ...
        for i in range(n_pt):
            ...
            if d < center_tol:
                ...
                matched_cls_ok.append(pt_pred["cls"][i] == om_pred["cls"][j])
                dim_err = np.abs(pt_pred["dim"][i] - om_pred["dim"][j]).max()
    ...
    def analyze_sample(self, sample_token, score_thresh=0.1, center_tol=2.0, atol_heatmap=1e-2):
        ...
        # 5. 保存结果
        result_file = os.path.join(self.output_dir, f'{sample_token}_analysis.json')
    ...
    def diagnose(self, results):
        ...
        if not results['heatmap_comparison']['is_match']:
            ...
            diagnosis['recommendations'].append("检查模型导出是否正确")
            diagnosis['recommendations'].append("检查ONNX转换是否引入误差")
        ...
        if not results['high_conf_comparison']['is_match']:
            ...
            diagnosis['recommendations'].append("检查Voxelization实现是否一致")
            diagnosis['recommendations'].append("检查点云预处理是否一致")
            diagnosis['recommendations'].append("检查coords格式转换是否正确")
    ...
    def visualize_results(self, data, sample_token):
        ...
        if len(om_heatmap.shape) == 4:
            ...
            im0 = axes[0].imshow(om_hm, cmap='hot')
            axes[0].set_title('OM Heatmap')
            ...
            diff_hm = np.abs(om_hm - pytorch_hm)
        ...
        for idx, name in enumerate(output_names):
            if name in data['om']['outputs'] and name in data['pytorch']['outputs']:
                ...
                axes[idx].scatter(om_data, pytorch_data, alpha=0.5, s=1)
                axes[idx].plot([om_data.min(), om_data.max()],
                              [om_data.min(), om_data.max()],
                              'r--', label='y=x')
                axes[idx].set_xlabel('OM')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='对比分析OM和PyTorch模型数据')
